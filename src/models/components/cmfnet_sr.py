...
class UEDM(nn.Module):
    ...
    def forward(self, ms, pan):
        ...
        fuse = 0
        for encoder, down, ms in zip(self.encoders, self.downs, ms_encs[:2]):
            # x = encoder(x)
        ...
        for decoder, up, ms in zip(self.decoders, self.ups, ms_encs[::-1][1:]):
            fuse = up(fuse)
            fuse = fuse + ms  # uedm6 skip+ms+pan
            fuse = decoder(fuse)
