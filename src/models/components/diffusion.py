...
def make_beta_schedule(schedule="sigmoid", n_timestep=1000, linear_start=1e-6, linear_end=1e-2, cosine_s=8e-3):
    if schedule == "quad":
...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, model, x, t, ms, pan, clip_denoised, return_pred_x0):

        model_output = model(x, t, ms, pan)
    ...
    def p_sample(self, model, x, t, ms, pan, noise_fn, clip_denoised=True, return_pred_x0=False):

        mean, _, log_var, pred_x0 = self.p_mean_variance(model, x, t, ms, pan, clip_denoised, return_pred_x0=True)
    ...
    @torch.no_grad()
    def p_sample_loop(self, model, ms, pan, noise_fn=torch.randn):
        b, C, h, w = ms.shape
        b, c, H, W = pan.shape
        shape = [b, C, H, W]
        ...
        for i in reversed(range(self.num_timesteps-999)):
            img = self.p_sample(
                model,
                img,
    ...
    def training_losses(self, model, x_0, t, ms, pan, noise=None):
        ...
        if self.loss_type == 'kl':
            ...
            model_output = model(x_t, t, ms, pan)
